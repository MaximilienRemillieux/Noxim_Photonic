...
void TargetPhotonic::b_transport( tlm::tlm_generic_payload& trans, sc_time& delay )
{
    struct Flit* my_flit = (struct Flit*)trans.get_data_ptr();

    LOG << "*** [Ch" <<local_id << "] Received photonic: " << *my_flit << endl;

    // only moves received flit to the photonic buffer
    // reservations stuff is done in the photonic hub to avoid
    // race conditions on shared reservation table
    if (!buffer_rx.IsFull())
    {
        LOG << "*** [Ch" <<local_id << "] Flit " << *my_flit << " moved to buffer_rx " << endl;
        buffer_rx.Push(*my_flit);
        photonichub->power.photonicBufferPush();
        // Obliged to set response status to indicate successful completion
        trans.set_response_status( tlm::TLM_OK_RESPONSE );
        //buffer_rx.Print();
    }
