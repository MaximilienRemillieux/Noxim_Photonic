...
SC_MODULE(Router)
{
    ...
    // number of ports: 4 mesh directions + local + wireless + Photonic
    sc_in <Flit> flit_rx[DIRECTIONS + 2 + 1];	  // The input channels 
    sc_in <bool> req_rx[DIRECTIONS + 2 + 1];	  // The requests associated with the input channels
    sc_out <bool> ack_rx[DIRECTIONS + 2 + 1];	  // The outgoing ack signals associated with the input channels
    sc_out <TBufferFullStatus> buffer_full_status_rx[DIRECTIONS + 2 + 1];

    sc_out <Flit> flit_tx[DIRECTIONS + 2 + 1];   // The output channels
    sc_out <bool> req_tx[DIRECTIONS + 2 + 1];	  // The requests associated with the output channels
    sc_in <bool> ack_tx[DIRECTIONS + 2 + 1];	  // The outgoing ack signals associated with the output channels
    sc_in <TBufferFullStatus> buffer_full_status_tx[DIRECTIONS + 2 + 1];
    ...
    int selection_type;
    BufferBank buffer[DIRECTIONS + 2 + 1];		// buffer[direction][virtual_channel] 
    bool current_level_rx[DIRECTIONS + 2 + 1];	// Current level for Alternating Bit Protocol (ABP)
    bool current_level_tx[DIRECTIONS + 2 + 1];	// Current level for Alternating Bit Protocol (ABP)
  ...
  private:
    ...
    int start_from_vc[DIRECTIONS + 2 + 1]; // VC from which to start the reservation cycle for the specific port
  ...
  public:
    ...
    bool connectedPhotonicHubs(int src_hub, int dst_hub);
