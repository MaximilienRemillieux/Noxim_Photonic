...
// Helper returning the ID of the nearest attached tile (hub entry point)
int Routing_HUB_FIRST::nearestHubNode(int node_id) {
    int best_node = -1;
    int min_dist = 1e9;
    for (auto &entry : GlobalParams::hub_configuration) {
        const HubConfig &hc = entry.second;
        for (int hub_node : hc.attachedNodes) {
...
// Distance vers hub le plus proche (Manhattan minimum sur tous les PEs attachés)
...
vector<int> Routing_HUB_FIRST::route(Router * router, const RouteData & routeData) {
    ...
    if (use_hub) {
        bool currHasHub = (GlobalParams::hub_for_tile.find(current) != GlobalParams::hub_for_tile.end());
        bool dstHasHub  = (GlobalParams::hub_for_tile.find(destination) != GlobalParams::hub_for_tile.end());

        // At a tile attached to a hub, we can inject into the wireless network.
        // Two cases:
        // 1. Destination itself is hub-attached: we just send to the hub port.
        // 2. Destination is not hub-attached: we must select a relay node
        //    (some tile connected to the hub closest to the destination) so that
        //    the hub knows where to forward the flit.
        if (currHasHub) {
            if (dstHasHub) {
                // common case: both ends are hub connected
                directions.push_back(DIRECTION_HUB);
                cout << "Current node " << current << " is using hub_entry "
                << GlobalParams::hub_for_tile.find(current)->first << " to destination hub_exit "
                << GlobalParams::hub_for_tile.find(destination)->first << " toward final destination "
                << destination << " VC = " << vc
                << endl;
                ...
                if (relay >= 0 && GlobalParams::hub_for_tile.find(relay) != GlobalParams::hub_for_tile.end()) {
                    // encode as DIRECTION_HUB_RELAY + tile_id so the router
                    // will record flit.hub_relay_node.
                    directions.push_back(DIRECTION_HUB_RELAY + relay);
                    cout << "Current node " << current << " is using hub_relay_entry "
                    << GlobalParams::hub_for_tile.find(current)->first << " to destination hub_relay_exit "
